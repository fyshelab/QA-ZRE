...
class QAT5(MyBaseT5):
    ...
    def relation_extraction_predict(
        self, batch: torch.utils.data.Dataset
    ) -> Iterator[Dict[str, str]]:
        """The main prediction loop for scoring every possible relation by
        computing the score for the gold tail entity."""
        ...
        labels = orig_labels.masked_fill(
            orig_labels == self.tokenizer.pad_token_id, -100
        )
        ...
        # tail_log_p now has the log probability of the full sequence for the example output: "positive"
        tail_log_p = torch.sum(good_log_p, dim=1).squeeze().cpu().detach().numpy()
        ...
        potentials_str = self.tokenizer.batch_decode(
            orig_labels, skip_special_tokens=True
        )
        ...
        for index, input_str in enumerate(inputs_str):
            output_row = {
                ...
                "relation_log_p": tail_log_p[index],
                "input": input_str,
            }
            yield output_row
