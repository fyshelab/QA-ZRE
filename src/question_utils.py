...
def create_data_for_question_pretrain():
    """This will be the data used for question pre-training."""
    ...
    for index, q in enumerate(train_questions):
        ...
        if len(q_ents) > 0:
            # consider questions with a tagged entity.
            ...
            for token in final_doc:
                if token not in all_entities:
                    # only select non-entity tokens from the questions.
    ...
    print("Number of questions for pre-training:", len(good_qs))
    contexts = []
    questions = []
    for row in good_qs:
        context = row[0]
        ...
        if len(tokens) >= 4:
            ...
            # sample at most four tokens.
            sampled_tokens = random.sample(tokens, token_num)
            relation_signal = " ".join(sampled_tokens)

        # format of the input for question pre-training.
        # this follows the format of the T5 for question generation.
        contexts.append(
            "answer: "
...
def create_question_pretrain_dataset(
    ...
    """Function to create the question input-outputs to do the pretraining of
    ...
    output_encodings = question_tokenizer(
        questions,
        truncation=True,
        padding="max_length",
    ...
    dataloader = DataLoader(QADataset(data), batch_size=batch_size, shuffle=shuffle)
    return dataloader
